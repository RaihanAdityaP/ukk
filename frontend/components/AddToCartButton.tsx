...
export default function AddToCartButton({
  ...
  const actualStock = maxStock ?? stock ?? 999
  const isButtonDisabled = disabled || actualStock === 0
  ...
  async function handleConfirm() {
    ...
    try {
      await api.cart.add(productId, finalQty)
      setLoading(false)
      setModalOpen(false)
      ...
      setModalOpen(false)
      setToast({ type: 'error', text: err.message || 'Gagal menambahkan ke keranjang.' })
      setTimeout(() => setToast(null), 3000)
    }
  }
  ...
  return (
    <>
      {compact ? (
        ...
        >
          <Plus className="w-4 h-4" />
          {isButtonDisabled ? 'Stok Habis' : 'Tambah'}
        </button>
      )}
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-6 pr-6">
              {imageUrl ? (
                ...
                <img src={getImageUrl(imageUrl)} alt={productName} className="w-14 h-14 rounded-lg object-cover flex-shrink-0" />
              ) : (
